...
class LLMClient:
    ...
    async def call_llm(self, model_name: str, prompt: str, system_prompt: str = None) -> dict:
        """
    ...
    async def call_gemini(self, prompt: str, model_name: str = None) -> dict:
        ...
        try:
            ...
            if '/' in model_name:
                raise ValueError(f"Invalid model name '{model_name}' for Gemini API. Should not contain '/'.")
            ...
            if response.parts:
                ...
                logger.warning(f"Gemini response blocked. Reason: {block_reason_message}")
                return {
    ...
    async def call_openrouter(self, model_name: str, prompt: str, system_prompt: str = None) -> dict:
        ...
        try:
            ...
            logger.error(f"HTTP error calling OpenRouter API ({model_name}): {e.response.status_code} - {error_text}", exc_info=True)
            return {
