...
class StorageService:
    def __init__(self):
        ...
        if settings.SUPABASE_URL and settings.SUPABASE_KEY:
            try:
                ...
                logger.info(
                    "Supabase client initialized for StorageService (Cloud Mode)."
                )
            except Exception as e:
                logger.error(
                    f"Failed to initialize Supabase client, falling back to local mode: {e}",
                    exc_info=True,
                )
            ...
            self.is_local_mode = True
            logger.info(
                "StorageService initialized in Local Mode. Files will be stored in './local_storage'."
            )
    ...
    def upload_file(self, bucket_name: str, file_path: str, file_content: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Successfully uploaded {file_path} to Supabase bucket {bucket_name}."
            )
            return True
        except Exception as e:
            logger.error(
                f"Failed to upload {file_path} to Supabase Storage: {e}", exc_info=True
            )
            return False
    ...
    def download_file(self, bucket_name: str, file_path: str) -> Optional[str]:
        ...
        try:
            ...
            logger.info(
                f"Successfully downloaded {file_path} from Supabase bucket {bucket_name}."
            )
    ...
    def create_bucket(self, bucket_name: str) -> bool:
        ...
        try:
            ...
            if "Bucket already exists" in str(e):
                logger.warning(
                    f"Supabase bucket '{bucket_name}' already exists. Skipping creation."
                )
                return True
            logger.error(
                f"Failed to create Supabase bucket {bucket_name}: {e}", exc_info=True
            )
            return False
    ...
    def list_files(self, bucket_name: str) -> List[dict]:
        ...
        try:
            ...
            logger.error(
                f"Failed to list files for Supabase bucket {bucket_name}: {e}",
                exc_info=True,
            )
            return []
    ...
    def _local_create_bucket(self, bucket_name: str) -> bool:
        try:
            ...
            logger.info(
                f"Local bucket '{bucket_name}' created/ensured at: {bucket_path}"
            )
            return True
        except OSError as e:
            logger.error(
                f"Failed to create local bucket '{bucket_name}': {e}", exc_info=True
            )
            return False

    def _local_upload(
        self, bucket_name: str, file_path: str, file_content: str
    ) -> bool:
        try:
            ...
            logger.error(
                f"Failed to upload local file to {full_path}: {e}", exc_info=True
            )
            return False
    ...
    def _local_download(self, bucket_name: str, file_path: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(
                f"Failed to download local file from {full_path}: {e}", exc_info=True
            )
            return None
