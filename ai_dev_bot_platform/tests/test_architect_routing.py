...
async def test_planning_task_routing(orchestrator, mock_user, mock_db):
    ...
    with patch.object(orchestrator.architect_agent, 'generate_plan', return_value={
        "plan_filename": "plan.md",
        "plan_content": "Sample plan content",
        "llm_call_details": {"model_name_used": "test-model", "input_tokens": 100, "output_tokens": 150}
        ...
        assert result["text"].startswith("Planning task 'Plan the architecture' completed!")
...
async def test_implementation_task_routing(orchestrator, mock_user, mock_db):
    ...
    with patch.object(orchestrator.implementer_agent, 'implement_todo_item', return_value={
        "filename": "app.py",
...
async def test_message_handler_routing(orchestrator, mock_user, mock_db):
    ...
    with patch("app.telegram_bot.handlers.is_in_requirement_gathering", return_value=False), \
