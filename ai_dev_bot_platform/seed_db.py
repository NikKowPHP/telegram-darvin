...
def seed_api_keys():
    ...
    try:
        ...
        print(
            f"Error: Invalid API_KEY_ENCRYPTION_KEY. It must be 32 url-safe base64-encoded bytes. Error: {e}"
        )
        return
    ...
    for provider, key_value in keys_to_seed.items():
        ...
        if exists:
            print(
                f"Key for provider '{provider}' already exists in the database. Skipping."
            )
        else:
            ...
            new_key = APIKey(
                provider=provider, encrypted_key=encrypted_key, is_active=True
            )
            db.add(new_key)
